...
Function RaylibConverter.sProcessDefine(ByVal sLine As String, ByRef bShouldPrint As Integer) As String
    ...
    If InStr(sLine, "RAYLIB_H") > 0 Or InStr(sLine, "RLGL_H") > 0 Or InStr(sLine, "RAYMATH_H") > 0 Then Return ""
    If InStr(sLine, "_VERSION") > 0 Then Return ""
    If InStr(sLine, "RLAPI") > 0 Or InStr(sLine, "RMAPI") > 0 Then Return ""
    ...
    If InStr(sLine, "EPSILON") > 0 Then
        If bIsRaymath Then Return "" ' Usually defined inside functions in C, ignore global redef
    End If
    ...
    If iSpace2 > 0 Then
        ...
        If sVal = "" Then Return ""
        If InStr(sKey, "(") > 0 Then Return "" ' Skip macros with args
...
Function RaylibConverter.sProcessStruct(ByVal iFileNum As Integer, ByVal sHeaderLine As String) As String
    ...
    If (bIsRlgl Or bIsRaymath) Then
        If sName = "Matrix" Or sName = "Vector2" Or sName = "Vector3" Or sName = "Vector4" Or sName = "Quaternion" Then
...
Sub RaylibConverter.Convert(ByVal sInputPath As String, ByVal sOutputPath As String)
    ...
    Do Until Eof(iIn)
        ...
        If InStr(sLine, "#define RLGL_VERSION") > 0 Then
            ...
            Exit Do
        End If
        ' Raymath doesn't always have a clear version string, but has header guard
    ...
    Do Until Eof(iIn)
        ...
        If Left(sLine, 14) = "typedef struct" Then
            If InStr(sLine, ";") > 0 Then
                ...
                If (bIsRlgl Or bIsRaymath) And (sName = "Matrix" Or sName = "Vector2" Or sName = "Vector3" Or sName = "Vector4" Or sName = "Quaternion") Then Continue Do
        ...
        If Left(sLine, 8) = "typedef " And InStr(sLine, "struct") = 0 And InStr(sLine, "enum") = 0 And InStr(sLine, "(*") = 0 Then
            ...
            ' Ignore Quaternion alias in Raymath (typedef Vector4 Quaternion)
            If bIsRaymath And sAlias = "Quaternion" Then Continue Do
