...
Sub RaylibConverter.Convert(ByVal sInputPath As String, ByVal sOutputPath As String)
  ...
  Else
    ...
    Print #iOut, ""
    Print #iOut, "  #ifndef __FB_64BIT__"
    Print #iOut, "    ' Compatibilidad de Linker 32-bit (FBC32 antiguo vs MinGW-w64 moderno)"
    Print #iOut, "    ' Evita error durante la vinculacion: undefined reference to 'stat64i32'"
    Print #iOut, "    #ifndef RAYLIB_DISABLE_STAT_FIX"
    Print #iOut, "    #ifndef RAYLIB_STAT_FIX_APPLIED"
    Print #iOut, "    #define RAYLIB_STAT_FIX_APPLIED"
    Print #iOut, "    extern ""C"""
    Print #iOut, "      function _stat64i32_wrapper cdecl alias ""_stat64i32"" (byval path as any ptr, byval buffer as any ptr) as long"
    Print #iOut, "        return -1"
    Print #iOut, "      end function"
    Print #iOut, "      function stat64i32_wrapper cdecl alias ""stat64i32"" (byval path as any ptr, byval buffer as any ptr) as long"
    Print #iOut, "        return -1"
    Print #iOut, "      end function"
  ...
  ' Utility for semantic conditions (isnot)"
  ...
  Do Until Eof(iIn)
    ...
    If Left(sLine, 8) = "typedef " And InStr(sLine, "struct") = 0 And InStr(sLine, "enum") = 0 And InStr(sLine, "(*") = 0 Then
      ...
      ' FIX RAYLIB 6.0+: Handle pointers in typedefs properly
      Dim iAliasPtrs As Integer = iCountChar(sAlias, "*")
      Dim iSrcPtrs As Integer = iCountChar(sSrc, "*")
      Dim iTotalPtrs As Integer = iAliasPtrs + iSrcPtrs
      
      If iTotalPtrs > 0 Then
        sAlias = sReplace(sAlias, "*", "")
        sSrc = sReplace(sSrc, "*", "")
        sSrc = Trim(sSrc)
        Dim i As Integer
        For i = 1 To iTotalPtrs
          sSrc += " ptr"
        Next
      End If
